...
setup(
    name='many_pynb', # パッケージ名(プロジェクト名)
    packages=['many_pynb'], # パッケージ内(プロジェクト内)のパッケージ名をリスト形式で指定
    ...
    url='https://github.com/toshiakiasakura/many_pynb', # パッケージに関連するサイトのURL(GitHubなど)

    description='Output various settings of jupyter notebook which kernel is python.', # パッケージの簡単な説明
    ...
    keywords='jupyter notebook papermill ipynb_path html', # PyPIでの検索用キーワードをスペース区切りで指定
